...
export const PopupMenu: FC<PopupMenuProps> = ({ isOpen, activeItem, changeIsOpen, changeActiveItem, popupItems }) => {
  ...
  return (
    <div className={styles.root} ref={sortRef}>
      <div className={styles.title} onClick={() => clickActiveItem()}>
        <span className={styles.text}>{popupItems[activeItem]}</span>

        <svg
          className={isOpen ? styles.svg : ''}
          width='10'
          height='6'
          viewBox='0 0 10 6'
          fill='none'
      ...
      {isOpen && <PopupItems popupItems={popupItems} clickItem={clickMenuItem} />}
    </div>
  );
};
